...
make_graph <- function(df, pop.value, plot.type) {
  ...
  dout_grouped <- dplyr::group_by(dout, bin_number)
  dd <- dplyr::summarise(dout_grouped, bin_size=n())
  print(dd)
...
df <- data.frame(x = rnorm(5000, mean = 100, sd = 15))
gg <- make_graph(df, pop.value = 100, plot.type = "happy_face")
print(gg)
